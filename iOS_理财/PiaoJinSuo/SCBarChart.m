...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    NSInteger rowCount = 6; // 自动计算每个图表适合的行数
...
-(void)strokeChart
{
    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        for (int j = 0; j<childAry.count; j++) {
            ...
            float grade = ((float)value - 6) / 6;
            
            SCBar * bar = [[SCBar alloc] initWithFrame:CGRectMake((j+(_yValues.count==1?0.1:0.05))*_xLabelWidth +i*_xLabelWidth * 0.47, UULabelHeight * 1.5, _xLabelWidth * (_yValues.count==1?0.8:0.45), chartCavanHeight)];
            bar.barColor = [_colors objectAtIndex:i];
            bar.grade = grade;
            bar.tag = i;
            bar.info = _yValues[i][j];
            [bar addTarget:self action:@selector(showInfo:) forControlEvents:UIControlEventTouchUpInside];
