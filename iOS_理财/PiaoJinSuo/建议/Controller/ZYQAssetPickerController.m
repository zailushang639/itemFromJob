...
+ (void)initialize
{
    ...
    checkedIcon     = [UIImage imageNamed:@"OK"];//红色狗狗
    
    checkedNoIcon     = [UIImage imageNamed:@"No"];//蓝色狗狗
...
@property (nonatomic, retain) ZYQTapAssetView *tapAssetView;//半透明膜
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        [self addSubview:_imageView];//这里的_imageView指相册中的照片代表的图像视图
...
- (void)bind:(ALAsset *)asset selectionFilter:(NSPredicate*)selectionFilter isSeleced:(BOOL)isSeleced
{
    ...
    [_imageView setImage:[UIImage imageWithCGImage:asset.thumbnail]];//_imageView(相册中的照片视图)加载照片
    ...
    //选中状态的前面的半透明的膜
    _tapAssetView.selected= isSeleced;//isSeleced是非选中状态
}
...
static ZYQAssetViewController *single = nil;

+ (ZYQAssetViewController *)shareManager
{
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        single = [[ZYQAssetViewController alloc] init];
        NSMutableAttributedString *attriString = [[NSMutableAttributedString alloc] initWithString:@"选择图片"];
        [attriString addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0, 4)];
        UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 200, 44)];
        titleLabel.textAlignment = NSTextAlignmentCenter;
        titleLabel.attributedText=attriString;
        single.navigationItem.titleView = titleLabel;
    });
...
- (void)localize
{
    ...
    NSMutableAttributedString *attriString = [[NSMutableAttributedString alloc] initWithString:@"相册"];
...
//    ZYQAssetViewController *vc = [[ZYQAssetViewController alloc] init];
    ZYQAssetViewController *vc = [ZYQAssetViewController shareManager];
