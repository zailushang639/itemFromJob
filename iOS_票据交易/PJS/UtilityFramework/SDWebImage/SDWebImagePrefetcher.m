...
- (void)reportStatus
{
    ...
    NSLog(@"Finished prefetching (%ld successful, %ld skipped, timeElasped %.2f)", total - _skippedCount, _skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
    NSLog(@"Prefetched %ld out of %ld", _finishedCount, [self.prefetchURLs count]);
...
{
    _finishedCount++;
    NSLog(@"Prefetched %ld out of %ld (Failed)", _finishedCount, [self.prefetchURLs count]);
