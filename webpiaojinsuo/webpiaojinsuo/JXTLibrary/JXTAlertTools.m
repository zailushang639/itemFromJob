...
+(void)alertStr:(NSString *)str withViewController:(UIViewController *)vc{
    [self showTipAlertViewWith:vc title:EmptyTitle message:str buttonTitle:nil buttonStyle:JXTAlertActionStyleDefault];
}
+(void)alertStrBottom:(NSString *)str withViewController:(UIViewController *)vc{
    [self showBottomTipViewWith:vc title:EmptyTitle message:str];
}
        ...
        if (btnTitle) {
            ...
            switch (btnStyle) {
                case JXTAlertActionStyleDefault:
                    ...
                    singleAction2 = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
          ...
          otherButtonTitles:(NSString *)otherButtonTitles, ...
{
    if (iOS_Version >= 8.0)
    {
        ...
        if (destructiveBtnTitle) {
            ...
            if ([destructiveBtnTitle isEqualToString:@"拍照"]) {
                destructiveAction = [UIAlertAction actionWithTitle:destructiveBtnTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    block(0);
                }];
            }
            else{
                destructiveAction = [UIAlertAction actionWithTitle:destructiveBtnTitle style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
                    block(0);
                }];
            }
            [alertController addAction:destructiveAction];
...
+ (BOOL)isAlertShowNow
{
    if (iOS_Version >= 8.0) { //同步上面的定义，8.0后默认用alertViewController，所以查找vc
        ...
        UIViewController * activityVC = [self activityViewController];//查找当前活动窗口
